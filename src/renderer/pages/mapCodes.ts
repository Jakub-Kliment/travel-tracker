/**
 * The tables that tie the bundled world atlas to our own country codes.
 *
 * These live outside MapPage so the alignment between the drawn geometry and
 * the country list can be asserted in a test: a code the atlas draws but the
 * list omits renders as inert land, which is invisible in review.
 */
...
export const countryIdToIso: { [key: string]: string } = {
...
export const territoryNameToIso: [test: (name: string) => boolean, iso: string][] = [
